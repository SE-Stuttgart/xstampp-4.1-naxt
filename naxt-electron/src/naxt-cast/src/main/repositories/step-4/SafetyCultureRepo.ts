...
@injectable()
export class SafetyCultureRepo extends Repository<SafetyCulture> {
  constructor(@inject(CastDBConnector) dbConnector: DBConnector) {
    super(SafetyCulture, CastBaseQuery.id, dbConnector);
  }

  async exists(projectId: string, id: string): Promise<boolean> {
    return super._exists({ ...new SafetyCulture(), projectId, id });
  }
}
