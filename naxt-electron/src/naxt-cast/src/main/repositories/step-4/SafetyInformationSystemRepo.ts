...
@injectable()
export class SafetyInformationSystemRepo extends Repository<SafetyInformationSystem> {
  constructor(@inject(CastDBConnector) dbConnector: DBConnector) {
    super(SafetyInformationSystem, CastBaseQuery.id, dbConnector);
  }

  async exists(projectId: string, id: string): Promise<boolean> {
    return super._exists({ ...new SafetyInformationSystem(), projectId, id });
  }
}
